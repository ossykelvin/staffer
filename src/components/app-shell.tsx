              ...
              {publicEnv.NEXT_PUBLIC_DEMO_MODE === "true" ? (
                ...
                <details className="group relative">
                  <summary
                    ...
                    className="grid size-9 cursor-pointer list-none place-items-center rounded-full border border-cyan-300/30 bg-cyan-300/10 text-xs font-bold text-cyan-100 transition hover:border-cyan-200/60 hover:bg-cyan-300/15 [&::-webkit-details-marker]:hidden"
                  >
                    {profileInitial(user.email)}
                  </summary>
                  <div className="absolute right-0 mt-3 w-64 rounded-2xl border border-white/10 bg-[#0b172d] p-3 shadow-2xl shadow-black/40">
                    ...
                    <Link href="/settings" className="mt-2 block rounded-xl px-3 py-2 text-sm font-medium text-slate-300 transition hover:bg-white/7 hover:text-white">
                      Settings
                    </Link>
                    <form action={signOutAction}>
                      <button type="submit" className="block w-full rounded-xl px-3 py-2 text-left text-sm font-medium text-rose-200 transition hover:bg-rose-400/10 hover:text-rose-100">
                        Sign out
                      </button>
                    </form>
                  </div>
